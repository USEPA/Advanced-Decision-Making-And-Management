...
class CaseStudy(models.Model):
    ...
    # supply data
    supLatLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    supLngLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    supProLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    supCapLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    supBidLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    supNames = ListTextField(base_field=models.CharField(max_length=10), default = [])
    #supply results
    supValueLs = ListTextField(base_field=models.CharField(max_length=10), default = [])

    # tech site data
    siteLatLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    siteLngLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    siteTecLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    siteCapLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    siteNames = ListTextField(base_field=models.CharField(max_length=10), default = [])
    # tech site results
    siteTreatLs = ListTextField(base_field=models.CharField(max_length=10), default = [])

    # tech cand data
    candLatLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    candLngLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    candTecLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    candNames = ListTextField(base_field=models.CharField(max_length=10), default = [])
    # tech cand results
    candInstallLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    candCapLs = ListTextField(base_field=models.CharField(max_length=10), default = [])

    # demand data
    demLatLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    demLngLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    demProLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    demCapLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    demBidLs = ListTextField(base_field=models.CharField(max_length=10), default = [])
    demNames = ListTextField(base_field=models.CharField(max_length=10), default = [])
    #demand results
    demValueLs = ListTextField(base_field=models.CharField(max_length=10), default = [])

    # Other results
    summary = models.CharField(max_length = 5000, default = '')
    transportationResults = ListTextField(base_field=models.CharField(max_length=1000), default = []) # list of tech file paths
    ...
    def copyfiles(self, task):
        ...
        newpath = settings.MEDIA_ROOT + '/casestudies/' + str(self.id) + '_' + self.name + '/'
        try:
            os.stat(newpath)
        except:
            os.mkdir(newpath)
    ...
    def delete(self, *args, **kwargs):
        try:
            path = settings.MEDIA_ROOT + '/casestudies/' + str(self.id) + '_' + self.name + '/'
    ...
    def getprodlist(self):
        folder = settings.MEDIA_ROOT + '/casestudies/' + str(self.id) + '_' + self.name + '/'
        prodlist = []
        try:
...
class OptTask(models.Model):
    ...
    task_pseudoid = models.CharField(max_length = 20, default = '00000000000000000000')
    ...
    node_path = models.CharField(max_length=200, default = 'NA') # data path, uploaded by users
    ...
    def getprodlist(self):
        ...
        try:
            with open(settings.MEDIA_ROOT + '/' + str(prodfile), newline='') as csvfile:
                data = list(csv.reader(csvfile))
            newdata = []
            for i in range(1,len(data)):
                try:
                    item = {'id': i, 'alia': data[i][0],'name':data[i][1],'transcost':float(data[i][2]), 'unit':data[i][4]}
                except:
                    item = {'id': i, 'alia': data[i][0],'name':data[i][1],'transcost':float(data[i][2]), 'unit': 'UNIT'}
                newdata.append(item)
    ...
    def gettechlist(self):
        ...
        try:
            with open(settings.MEDIA_ROOT + '/' + str(techfile), newline='') as csvfile:
                data = list(csv.reader(csvfile))
            newdata = []
            for i in range(1,len(data)):
                refprod = None
                ...
                for item in prodls:
                    if data[i][4] == item['alia']:
                        refprod = item['name']
                        unit = item['unit']
                item = {'id': i, 'alia': data[i][0],'name': data[i][1], 'capmin':float(data[i][2]),'capmax':float(data[i][3]), 'refprod': refprod, 'K_inv': float(data[i][5]), 'B_inv': float(data[i][6]), 'K_op': float(data[i][7]), 'B_op': float(data[i][8]), 'unit':unit}
                newdata.append(item)
    ...
    def delete(self, *args, **kwargs):
        try:
            path = settings.MEDIA_ROOT + '/' + str(self.user.id) + '_' + self.user.username + '/task_' + str(self.id) + '/'
            for root, dirs, files in os.walk(path):
                for filename in files:
                    ff = path + filename
                    os.remove(ff)
...
class Technology(models.Model):
    ...
    def yields(self):
        ...
        for p in self.prods.all():
            trans = Transformation.objects.filter(technology = self, product = p)[0]
